...
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
  ...
  class StatusCreator{
    ...
    _createStatus(statusType){
      throw new console.error("Method '_createStatus' must be implemented.");
    }
  }

  class TicketStatusCreator extends StatusCreator{
    _createStatus(statusType){
      const normalizedStatus = statusType?.toLowerCase() || "unknow";
      switch (normalizedStatus) {
        case "resolved": return new ResolvedStatus();
        case "accepted": return new AcceptedStatus();
        case "pending": return new PendingStatus();
        case "rejected": return new RejectedStatus();
        default: return new UnknownStatus();
      }
    }
  }
  ...
  class PendingStatus extends Status {
    getColor() {return "bg-red-500/20 text-red-400 border-red-500/30";}
    getIcon() {return faHourglass;}
    getBackgroundGradient() {return "from-red-500/20 to-red-600/30 text-red-400";}
  }

  class AcceptedStatus extends Status {
    getColor() { return "bg-yellow-500/20 text-yellow-400 border-yellow-500/30"; }
    getIcon() { return faCheckCircle; }
    getBackgroundGradient() { return "from-yellow-500/20 to-yellow-600/30 text-yellow-400"; }
  }
  
  class ResolvedStatus extends Status {
    getColor() { return "bg-green-500/20 text-green-400 border-green-500/30"; }
    getIcon() { return faCheck; }
    getBackgroundGradient() { return "from-green-500/20 to-green-600/30 text-green-400"; }
  }
  
  class RejectedStatus extends Status {
    getColor() { return "bg-gray-500/20 text-gray-400 border-gray-500/30"; }
    getIcon() { return faTimesCircle; }
    getBackgroundGradient() { return "from-gray-500/20 to-gray-600/30 text-gray-400"; }
  }
  
  class UnknownStatus extends Status {
    getColor() { return "bg-indigo-500/20 text-indigo-400 border-indigo-500/30"; }
